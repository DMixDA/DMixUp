...
def save_net(args, models, type):
    ...
    torch.save(net.module.state_dict(), save_folder + '/' + 'net' + str(type) + '.pt')
    torch.save(head.module.state_dict(), save_folder + '/' + 'head' + str(type) + '.pt')
    torch.save(classifier.module.state_dict(), save_folder + '/' + 'classify' + str(type) + '.pt')
...
def evaluate(models, loader):
    ...
    print('AccuracyRate: {:.2f}%'.format((correct / total) * 100))
...
def get_dmix_loss(net, src_imgs, tgt_imgs, src_labels, tgt_pseudo, ratio):
...
def final_eval(models_sd, models_td, tgt_test_loader):
    ...
    print('Final Accuracy Rate: {:.2f}%'.format((correct / total) * 100))
